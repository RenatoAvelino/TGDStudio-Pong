...
public class BallScript : MonoBehaviour
{
    ...
    public int testHelper; //Used to assist in testing void methods
    public bool isTest = false;
    // Start is called before the first frame update
    void Start()
    {
        GetComponent<Rigidbody>().velocity = Vector3.right * speed; //Move the ball when the game starts
    }
   ...
   void OnCollisionEnter(Collision collision) //Checks whether the ball has collided
   {
        if(collision.gameObject.tag == "Player") //Checks whether the ball hit a Player
        {
            ...
            if (isTest)
            {
                ...
                if (collision.gameObject.GetComponent<BatonScript>().axis == 1) testHelper *= -1;
                return;
            }
            float newXDir = 1;
            if (collision.gameObject.GetComponent<BatonScript>().axis == 1) newXDir = -1;
